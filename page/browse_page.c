...
/*************************************************
*函数名称: GetSelectDir
*功能描述: 在"浏览页面"中,用户可能会选中某个目录 -- 它就是"连播页面"要显示的目录，本函数返回这个目录名
*输入参数: 无
...
*
**************************************************/
void GetSelectDir(char *strSeletedDir)
{	
...
/*************************************************
*函数名称: CalcBrowsePageLayout
*功能描述: 计算页面中各图标的坐标值
*输入参数: 无
*输出参数: ptPageLayout - 内含各图标的左上角/右下角坐标值
*返回值  : 0            - 成功
		 : 其他值       - 失败
*
*修改日期    版本号     修改人       修改内容
...
/*************************************************
*函数名称: CalcBrowsePageDirAndFileLayout
*功能描述: 计算"目录和文件"的显示区域
*输入参数: 无
*输出参数: 无
*返回值  : 0            - 成功
...
*
**************************************************/
static int CalcBrowsePageDirAndFileLayout(void)
{
	int iXres, iYres , iBpp;
	int iTopLeftX , iTopLeftY ;
	int iTopLeftXBak;
	int iBotRightX , iBotRightY;
	int iIconWidth , iIconHegiht;
	...
	if(iXres < iYres)
	{
		...
		iTopLeftX  = 0;
		iBotRightX = iXres - 1;
		iTopLeftY  = g_atBrowsePageIconsLayout[0].iBotRightY + 1;
		iBotRightY = iYres -1;
	}
	else
	{
	...
	/*确定一行有多少个"目录和文件",显示多少行*/
	iIconWidth  = DIR_FILE_NAME_WIDTH;
	iIconHegiht = iIconWidth;
	...
	iNumPerCol = (iBotRightY - iTopLeftY + 1)/iIconHegiht;
	while(1)
	{
		iDeltaY = (iBotRightY - iTopLeftY + 1) - iIconHegiht * iNumPerCol;
...
*功能描述: 为"浏览页面"生成"目录和文件"区域中的图标和文字,就是显示目录内容
*输入参数: iStartIndex        - 在屏幕上显示的第一个"文件和目录"是aptDirContents数组里的哪一项 
...
static void ShowBrowsePage(PT_PageLayout ptPageLayout)
{
	...
	if(atLayout[0].iTopLeftX == 0)
	{
		...
		CalcBrowsePageDirAndFileLayout();
	}
...
*功能描述: 改变显示设备上的"目录或文件"的图标,表示"已经被选中"
		 : 对于目录图标,把他改为"file_opened图标"
		 : 对于文件图标,只是把图标反色
*输入参数: iDirFileIndex - 选中"目录和文件"区域中的那个图标
*输出参数: 无
*返回值  : 无
*
*修改日期    版本号     修改人       修改内容
...
/*************************************************
*函数名称: BrowsePageRun
*功能描述: "BrowsePage页面"的运行函数:显示菜单图标,显示图片,根据用户点击做出处理
		 : "浏览页面"有两个区域:菜单区域,目录和文件图标
		 : 为统一处理,"菜单图标"的序号为0,1,2,3,....., "目录和文件图标":的序号为1000,1001,...
		 
*输入参数: ptParentPageParams - 是源文件中调用Page()->Run函数时，传入的参数，
*							  - 可以用来传递上一个页面想要给下一个页面运行时需要的参数,
*输出参数: 无
*返回值  : 无
*
*修改日期    版本号     修改人       修改内容
...
static void BrowsePageRun(PT_PageParams ptParentPageParams)
{
	...
	int bIconPressed = 0;                /*界面上是否有图标显示为被按下*/
		...
		if(tInputEvent.iPressure == 0)
		{
			...
			if(bIconPressed)
			{
				if(iIndexPressed < DIRFILE_ICON_INDEX_BASE) /*菜单图标*/
				{
					...
					if(iIndexPressed == iIndex) /*按下松开都是同一个按钮*/
					{
						switch(iIndex)
						{
							case 0:  /*向上按钮*/
				...
				else
				{
					...
					else if(bHaveClickSelectIcon)
					{
						...
						if(g_aptDirContents[iDirFileContentIndex]->eFileType == FILETYPE_DIR)
						{
							ReleaseButton(&g_atBrowsePageIconsLayout[1]);
							bHaveClickSelectIcon = 0;
