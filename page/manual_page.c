...
*功能描述: 计算"manual页面"中菜单的
*输入参数: 无
*输出参数: ptPageLayout      - 内含各个图标的坐标布局值
*返回值  : 无
...
*功能描述: 计算"manual页面"中图片的显示区域
*输入参数: g_atManualMenuIconsLayout   - Manual页面菜单的布局参数(全局变量)
*输出参数: g_tManualPictureLayout      - 图片布局的全局变量
*返回值  : 无
...
*输出参数: g_tOriginPicPixelDatas  - 存入原始数据的全局变量
*返回值  : 非NULL                  - 一个PT_PixelDatas结构体指针
		   NULL                    - 失败           
...
*功能描述: 获得缩放后的图片像素数据
*输入参数: ptOriginPicPixelDatas   - 原始的像素数据
		   iZoomedWidth            - 要缩放的宽度
		   iZoomedHeight           - 要缩放的高度
*输出参数: g_tZoomedPicPixelDatas  - 要存入变换后数据的全局变量
*返回值  : 非NULL                  - 一个PT_PixelDatas结构体指针
		   NULL                    - 失败           
...
/*********************************************************
*函数名称:ShowManualPage
*功能描述:显示"manual页面" - 除了显示菜单图标外还会显示图片
 ...
 * 功能描述： 为了"manual页面"获得输入数据，判断输入事件位于哪个图标上
 * 输入参数： ptPageLayout  - 内含多个图标的显示区域	  
 * 输出参数： ptInputEvent  - 内含得到的输入数据
 * 返 回 值： -1            - 输入数据不位于任何一个图标上
 ...
 * 功能描述： 计算两个触点的距离，为了简化计算，返回距离的平方值
