...
*功能描述: 在"interval页面中",用户可能设置图片的显示间隔
		 : 本函数返回这个时间间隔
*输入参数: 无 
...
/*************************************************
*函数名称: IntervalPageCalcLayout
*功能描述: 计算页面中各图标的坐标值
*输入参数: 无 
*输出参数: ptPageLayout     - 内含各个图标的左上角/右下角值
*返回值  : 无
*修改日期    版本号     修改人       修改内容
*---------------------------------------------
*2019/8      V1.0       李德涛       创建
...
static void  IntervalPageCalcLayout(PT_PageLayout ptPageLayout)
{
	...
	/*
	* time.bmp 原图大小为128*72, 里面的两个数字大小为52*40
	* 
	*/
...
*返回值  : -1               - 输入数据不位于任何一个图标上
*		 : 其他值           - 输入数据所落在的图标(ptPageLayout->atLayout)
*修改日期    版本号     修改人       修改内容
*---------------------------------------------
*2019/8      V1.0       李德涛       创建
...
*功能描述: 为"interval"页面"生成特殊图标":绘制图表中的数字
...
*功能描述: "interval页面"的运行函数:显示菜单图标,显示图片,根据用户点击做出处理
	...
	while (1)
	{
		...
		else
		{
			...
			if (iIndex != -1)
			{
				...
				/*
				* 如果按下的是inc.bmp 或 dec.bmp
				* 如果2秒后，飞快的递增或减小:每50ms变化一次
				*/
